            ...
            <ul>
              <li v-for="entry in sessions[userId+'#'+currentSession.id]">
                ...
                <div class="main" :class="{self:entry.self}">
                  ...
                  <img class="avatar" :src="entry.self ? avatar: entry.fromProfile" alt="">
                  <p v-if="entry.messageType=='text'" class="text">{{ entry.content }}</p>
                  <img v-if="entry.messageType=='img'" :src="entry.content" class="img">
                </div>
              </li>
            </ul>
    </div>
    <div v-else>
      <ul>
        <li v-for="entry in sessions['群聊']" :key="entry.id">
          ...
          <div class="main" :class="{self:entry.fromId==userId}">
            <p class="username">{{ entry.fromName }}</p>
            <img @dblclick="takeAShot" class="avatar" :src="entry.fromProfile ? entry.fromProfile : '' " :onerror="defaultSrc" alt="">
            <div v-if="(entry.messageType=='text')"><p class="text" v-html="entry.content"></p></div>
            <div v-else>
              <!--图片预览与无法加载图片的图标-->
...
export default {
  ...
  data() {
    return {
      defaultSrc :  'this.src="' + require("@/assets/images/profile.jpg") + '"'
    }
  },
  computed: {
    ...mapGetters([
    ...
    ...mapState({
      ...
      errorImgUrl: state => state.chat.errorImgUrl,
    })
  },
  filters: {
    time(date) {
